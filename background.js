...
async function baiduTranslate(text) {
  try {
    ...
    if (!config.baiduAppId || !config.baiduKey) {
      throw new Error('请先在插件配置中设置百度翻译的 APP ID 和密钥');
    }
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === "translateText" && tab && tab.id) {
    console.log('Translating text:', info.selectionText);
    ...
    try {
      ...
      const translation = await baiduTranslate(info.selectionText);
      console.log('Sending translation to content script:', translation);
